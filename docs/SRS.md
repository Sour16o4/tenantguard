...
| **M3 — Tier 0 + second target** | `US-08`. ~~`TGD-BL-06` ceiling baselined~~ — **done** (§7.1, ratcheted §7.3). ~~`US-08`~~ — **done** (§7.4: `internal/triage`, `tenantguard triage`, measured against `coder`). ~~`TGD-BL-04`~~ — **resolved, not in the direction hoped**: `casdoor`'s actual test harness runs exclusively against MySQL (`.github/workflows/build.yml`'s `mysql:5.7` service, both the `go-tests` and `e2e` jobs; default `conf/app.conf` is `driverName = mysql`), verified by reading the harness itself, not `go.mod` (whose `lib/pq` dependency reflects `xorm`'s multi-driver support, not what the tests exercise). `casdoor` is disqualified per §3.2 (`C-1`, MySQL out of scope) — the wrong database engine, not an unverified one. **M3 needs a different second target**; two unverified candidates named (design §10): `go-gitea/gitea` (its own CI runs a real `postgres:14` integration-test job — an actually-exercised path, not a dependency) and `mattermost/mattermost`'s server (PostgreSQL-primary since MySQL's deprecation). Neither has had `casdoor`-level tenant-column verification yet. M3 as a whole is not yet closed — the second-target gap moved, it did not close. |
