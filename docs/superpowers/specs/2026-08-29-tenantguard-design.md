...
`CODER_PG_CONNECTION_URL` for `coder`, not `DATABASE_URL` (§10.2). Gate 0 has
...
|---|---|---|
| **`coder/coder`** | See §10.2 — precondition (a) met, injection point confirmed, TLS not an obstacle. | Precondition (b) resolved by §9.4 rather than by the target. **First target.** |
...
| **`zitadel/zitadel`** — `instance_id` only; real pipeline run, `infer` proven, `verify` blocked on `TGD-BL-46` (SRS §7.15) | PostgreSQL-only, driver `pgx` v5.9.2 confirmed against real traffic (`stmtcache_<sha256>` naming, `pgxpool`'s own cache). `instance_id` is a genuine hard partition — `resource_owner` out of scope (SRS §7.13). All five checklist questions passed (SRS §7.14). Real `infer` against the real database: 121/144 relations scoped correctly, a genuine composite `instance_id`/`org_id` tenancy shape correctly caught `Unclassifiable`, not mis-scoped. | `TGD-BL-07` fix landed and proven both synthetically and against the live schema — CLOSED. `TGD-BL-46` (new, filed, not fixed): `verify` cannot prove any of `zitadel`'s 121 scoped relations because none live in schema `public` and `CreateRestrictedRole` only grants there — `coder`/`casdoor` never exercised this. `audit` was not attempted; blocked on `TGD-BL-46`. |
| **`go-gitea/gitea`** ⚠ **DISQUALIFIED** | *[read-only]*: Postgres is genuinely, heavily exercised — `pull-db-tests.yml`'s `test-pgsql-shard-1`/`-2` (gated on `files-changed.outputs.backend=='true'`, i.e. any backend-touching PR) run `GITEA_TEST_DATABASE=pgsql make test-integration` with `-race`, sharded across two runners, plus `make test-migration` — the heaviest-loaded test path in the repo, not decorative. Driver is `lib/pq` (`go.mod` and `models/db/engine.go`'s blank import both confirm it), which this tool already supports (`C-5`, `[E]`). `tests/pgsql.ini.tmpl` sets `SSL_MODE = disable` — no TLS obstacle. Fixture density is fine: `models/fixtures/repository.yml` spreads rows across 10+ distinct `owner_id` values (14, 4, 4, 4, 3, 3...) in one table — `coder`'s 2-of-22 sparse-table problem would not recur here. | **Disqualifying finding: `owner_id` is not a tenant-isolation column.** `models/perm/access/repo_permission.go` computes visibility in Go, not SQL: `minAccessMode := util.Iif(!repo.IsPrivate && !user.IsRestricted, AccessModeRead, AccessModeNone)` — every non-restricted user gets read access to every non-private repo, regardless of `owner_id`, by design. Private-repo access layers team membership and collaborator grants on top, computed by walking `organization.GetUserRepoTeams`/`GetTeamUserIDsWithAccessToAnyRepoUnit`, not a single `WHERE owner_id = X`. This is a general multi-user ACL system (public/private + team/collaborator permissions + admin override), not shared-schema tenant isolation — cross-owner visibility is the *normal*, correct case (every public repo), not a violation. Synthesizing an RLS policy on `owner_id` would be actively wrong, not merely unverified: it would deny legitimate public/collaborator access and report it as a "finding," the same category of scope violation §3.2 already excludes for schema-per-tenant ("a different detection problem; claiming it would be a lie of scope") — `gitea` fits that exclusion for a different structural reason. Connection injection was not investigated further once this was found (INI-templated `HOST`/`NAME`/`USER`/`PASSWD`, not a single-var DSN like `coder`'s — moot given the disqualifier). |
| **`mattermost/mattermost`** (`server/`) ⚠ **DISQUALIFIED** | Go server, PostgreSQL-primary since MySQL support was deprecated in recent major versions. `TeamId` appears across many tables — a real column, not a naming coincidence. | See SRS §7.11: `TeamId` is app-computed visibility, the identical shape that disqualified `gitea`. Screening stopped at question 1 (§10.1) — the remaining four questions were not spent on a target already eliminated. |

### 10.1 Target-screening checklist

Five questions, in this order deliberately — the first is disqualifying on
its own and makes the other four moot if it fails, so it is asked before
any of them, not after. `casdoor` failed on question 2 (a different,
earlier-discovered failure mode, before this checklist existed in this
form); `gitea` and `mattermost` both failed on question 1. A candidate is
not screened further once a question fails it — the point of ordering is
precisely to avoid spending a session on questions 2–5 when question 1
already disqualifies.

1. **Real row-scoped tenant column, or app-computed visibility?** Read
   the actual authorization/permission code path, not the schema alone —
   a plausible-looking column (`owner_id`, `TeamId`, `org_id`) is not
   evidence by itself; `casdoor`'s `owner` passed this question genuinely
   (it disqualified on question 2 instead), while `gitea`'s `owner_id`
   and `mattermost`'s `TeamId` both looked equally plausible from the
   schema and both failed once the actual permission-check code was
   read. The disqualifying shape: visibility computed in application
   code as roles, group/team membership, or an admin/superuser bypass,
   where legitimate access crosses the "tenant" boundary as the normal,
   correct case (a public resource, a system role) rather than always
   being a violation. See SRS §3.2's "Access-control applications"
   exclusion — this is the check that catches it before any pipeline
   work goes in.

2. **Does the target's own CI actually run PostgreSQL, and is the job
   real rather than decorative?** Read the workflow file and any
   composite actions it calls, not `go.mod` — a Postgres driver
   dependency proves nothing about which engine the tests exercise
   (`casdoor`'s `lib/pq` dependency was present but unused by any real
   test job; its actual, exclusively-run harness was MySQL).

3. **Which driver — `pgx` or `lib/pq`?** Determines statement-naming
   behavior this tool's capture layer must already handle (`C-5`); not
   disqualifying either way since both are supported, but worth knowing
   before building against a target.

4. **Does the target's own test/fixture data seed more than one tenant
   with rows in the same table?** A tenant column that is real but
   sparsely populated in test fixtures reproduces `coder`'s own 2-of-22
   structural-only problem (§9.6) — checked directly against fixture
   files, not assumed from schema alone.

5. **Connection-URL injection point, and does it demand TLS?** Whether
   the target takes a single env-var DSN the way `coder`'s
   `CODER_PG_CONNECTION_URL` does, or something more indirect (config
   file templating), and whether `sslmode`/`SSL_MODE` defaults to
   requiring TLS (`TGD-NFR-12`, Tier 1 unsupported if so).

### 10.2 `coder/coder` — findings from Gate 0
...
|---|---|---|---|
| `TGD-BL-01` | **CLOSED — decided: keeping the name** | Name availability check for `tenantguard`. Run against GitHub, `pkg.go.dev`, and npm — see SRS §7.8: 36 existing GitHub repos of the same/similar name, an existing Go module sharing the identical `cmd/tenantguard` binary name, and a published `tenantguard-cli` npm package in the same problem space. **Decision, not just the check, is recorded in SRS §7.8**: the npm collision is a different ecosystem, no possible Go build collision, no legal issue found — the only real cost is search-result confusion, which does not justify cascading a rename through the `TGD-*` prefix across both spec documents. Name kept. | — |
...
| `TGD-BL-07` | **CLOSED — fix landed, proven red-before-green plus a mutation, and re-verified against the real `zitadel` schema** | Policy inference against `zitadel`'s versioned/multi-schema layout. See SRS §7.14/§7.15: versioned table naming (`users14`) and split `eventstore`/`projections`/`system` schemas confirmed non-issues, both synthetically and against the live database. `instance_id` added to `schema.Classify`'s candidate list (`resource_owner` deliberately excluded); `infer` run for real against `zitadel` classified 121/144 relations scoped correctly, including a genuine composite `instance_id`/`org_id` tenancy shape (`auth.org_project_mapping` and others) correctly caught as `Unclassifiable` rather than mis-scoped — a constraint shape `coder` never presented. Backs `TGD-US-09` AC-4, tracked under `M4`. | Target 3 |
| `TGD-BL-46` | **CLOSED — both fixes landed, each proven red-before-green plus a mutation** | Found running the real pipeline's `verify` step against `zitadel` (SRS §7.15). Two connected defects, both fixed (SRS §7.16): (1) `internal/oracle/probe.go`'s `CreateRestrictedRole` now takes `relations []schema.Relation` and grants `USAGE`/`SELECT`/`INSERT`/`UPDATE`/`DELETE`/sequence privileges on every distinct schema among them (plus `public`, always), not a hardcoded `public` alone — `TestCreateRestrictedRoleGrantsNonPublicSchema` reproduces `zitadel`'s exact `adminapi.styling2` shape, failed before, passes after, mutation-reverted-and-restored. (2) `cmd/tenantguard/oracle_gate.go` gained `nameUnderlyingTableError`, a pure function (mirroring `aggregateProof`'s own pattern) that surfaces a table's real recorded `A1Err`/`A4Err` in the operator-facing message when `PolicyProven()`'s generic `ErrA1`/`ErrA4` fires, instead of discarding it — `TestNameUnderlyingTableError_SurfacesRealCause`/`_NoRealCauseKnownIsUnchanged`, same red-before-green-plus-mutation discipline. Full pipeline re-run against real `zitadel` afterward: `verify` now succeeds (`proven: true`, 14/121 row-level, 107 structural-only — single real instance only, sampling's 2-row bar unmet on most per-instance-singleton config tables); `audit` over the preserved 5,167-event capture ran to completion and breached the baselined ceiling (`122/378≈0.3228` vs. measured `480/1187≈0.4044`), **not ratcheted, per instruction** — see SRS §7.16 for the full breakdown and why most of the breach is `TGD-BL-35`'s already-known duplicate-key replay limitation at much larger scale (a migration-heavy capture, not steady-state app traffic). | — |
| `TGD-BL-47` | **CLOSED — `TGD-NFR-03` ratcheted to `18.0/762.0`, gate re-proven on the new constant** | `TGD-BL-35`'s fix (SRS §7.18) lets `TGD-NFR-03` be honestly re-baselined for the first time by a real capability improvement rather than a coverage change or a measurement correction — the fourth baseline this project has recorded, and the first of that kind (SRS §7.19). Set: `unattributableCeilingRate = 18.0/762.0 ≈ 0.023622`, measured on `coder`'s fresh post-fix capture (`infer`/`verify` matched the historical `132/22/95/15`/`13-of-22-row-level` baseline exactly, confirming like-for-like traffic). Gate re-proven able to fire on the new number specifically, not assumed carried over from `TGD-BL-06`'s original proof: unit tests re-run and two fixed comparison rates lowered (`0.10`→`0.01`, `0.05`→`0.005`) after both were found to now sit above the new, much lower ceiling; end-to-end CLI tests re-run and two fixtures' `SAFE`-event counts raised (5→50, 4→50) for the same reason; the constant temporarily mutated to `0.30` and reverted, confirming the same `0.10` rate flips pass↔fail with it; the enforcement call in `main.go` temporarily wrapped in `if false` and reverted, confirming `TestAuditCLI_UnattributableCeilingFailsAboveIt` goes red without it. `zitadel`'s own post-fix 17.19% (SRS §7.18) clears the OLD ceiling but breaches this one — recorded in advance as expected (bootstrap-heavy capture vs. a steady-state-calibrated ceiling), not a future regression to chase. Full suite re-verified clean after every mutation and the final revert. | — |
| `TGD-BL-45` | **WITHDRAWN from `v1.0.0` (SRS §7.20), not built and not deferred again** | A full `TGD-NFR-02` measurement (Tier 1 time-to-first-finding) against a genuinely unfamiliar target, filed by `TGD-BL-05`. Assessed in two parts, each withdrawn for its own reason: the live-capture-to-first-finding phase is mechanically measurable by an agent session but needs a *third* real target neither `coder` nor `zitadel` (both now the most-examined repos in this project's history) — a real, boundable cost on the scale of the `zitadel` integration effort already completed, not started this session; the human policy-review phase requires an actual human operator to time, which no agent session can honestly substitute for without misrepresenting what was measured under `TGD-NFR-02`'s own name — a structural limitation, not a scheduling one, following the same precedent `TGD-BL-05` itself set (closed "measured as far as this environment honestly allows"). | `v1.1+`, split into its two real parts: a third-target live-capture timing (schedulable), and a genuine human-operator review-time study (needs a person). |
| `TGD-BL-48` | **CLOSED — test-isolation defect fixed, proven red-before-green, no product defect found** | `TestVerifyCLI_ProbeDatabaseIsAlwaysDropped` (`cmd/tenantguard/main_test.go`) intermittently failed under `-race` when run alongside `internal/oracle`'s own tests against the same PostgreSQL cluster (SRS §7.22). Root cause: the test's own `countProbes` helper matched `pg_database` on the cluster-wide pattern `tgd_probe_%`, which also counts `internal/oracle`'s differently-shaped `tgd_probe_<word>` fixture probes (a separate test binary, its own lifecycle) alongside the CLI's own `tgd_probe_<nanosecond-digits>` probes — not a probe leak, and not a `§3.3` violation; confirmed directly that no `tgd_probe_*` database survives a full run. Fixed by narrowing the predicate to a regex anchored to the CLI's own all-digits naming (`^tgd_probe_[0-9]+$`), which no `internal/oracle` fixture name can match. Reproduced red (reverted predicate, real failure, identical message) and confirmed green (restored predicate, repeated clean runs of the same unfiltered two-package combination) against real PostgreSQL; full suite and `scripts/run_tests_with_assertions.sh` both clean, zero skips. Latent on `main`; the `fix/stale-help-text` PR (text-only) exposed it by scheduling luck, did not introduce it. | — |
| `TGD-BL-09` | **CLOSED — both targets checked directly** | `SET`/`set_config` tenant context. `coder`: confirmed **no** *[read-only]*, so L5 stays a Tier 1 blind spot there. `zitadel`: confirmed **no** (SRS §7.14) — a full checkout grepped for `set_config`/`current_setting`/`SET`, the one hit (`setAppName`) sets `application_name` for observability only; tenant filters are ordinary bound query parameters, same as `coder`. L5 stays a Tier 1 blind spot for `zitadel` too — `LIMITATIONS.md` updated, not narrowed. | §7.4 scope |
...
| `TGD-BL-35` | **CLOSED — fixed, proven red-before-green plus a mutation, verified against both measured real-target shapes (SRS §7.18)** | **Finding F from `TGD-BL-33`'s categorization**, real captured `INSERT`s that attribute correctly and reach `diffWrite`, but whose `unrestricted` re-execution leg itself fails with Postgres `23505` (duplicate key) because the probe is a `TEMPLATE` copy of the *live* target database (§9.6/`TGD-BL-30`) and already contains the literal row the captured write's key collides with. Two independent measurements before the fix: `coder` (a bounded minority slice) and `zitadel` (277 of 480, 57.7%, of the target's entire `row_level_touching_real_app_sql` UNATTRIBUTABLE population — corrected from an initial, over-attributed 449/480 figure that conflated it with an unrelated undecoded-binary-parameter limitation; see SRS §7.16/§7.17/§7.18). **Fixed via a narrower, structurally-equivalent variant of the first design option below ("Option 1-lite," SRS §7.18):** rather than a capture-time point-in-time snapshot, the probe leg identifies the exact colliding row via Postgres's own structured `23505` error fields (`*pq.Error.Schema/.Table/.Detail`), deletes it inside the same `SAVEPOINT`-protected, already-rolled-back transaction, and retries the statement once — reaching the same structural result (test the write as if the colliding row were absent) without new snapshot infrastructure, and never durably writing to the target, so `§3.3` holds unchanged. The restricted leg attempts the identical delete first; if RLS does not admit it (the pre-existing row belongs to a different tenant), the result is `Unattributable`, never `Safe` or `Leak` — the safety constraint the fix was required to hold. Proven with three new tests (`TestDiff_DuplicateKeyOnReplay_SingleColumnKey_Resolves`, `_CompositeKey_Resolves`, `_RLSBlocksResolution_NeverSafe`), a caught mutation, and full re-runs of both targets: zitadel 40.44%→17.19%, coder fresh capture 2.36%, both under the (unmoved) ceiling. `TGD-NFR-03` re-baseline to `18/762≈2.36%` recommended in SRS §7.18, not set. | Originally: a design review of write re-execution against a probe seeded with real historical data — resolved without the full snapshot-based version of that review; the point-in-time-snapshot approach remains available as a future option if the surgical delete-and-retry mechanism is ever found insufficient (e.g. an ordering dependency across multiple writes in one capture, not observed on either target measured so far). |
