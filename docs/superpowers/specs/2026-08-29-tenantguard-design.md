...
| **`casdoor/casdoor`** | `owner` is part of the composite primary key on entities and is consistently used to filter (`Find(&permissions, &Permission{Owner: owner})`). A genuine shared-schema tenant column. | Whether its suite runs PostgreSQL or MySQL — `TGD-BL-04`. |
...
|---|---|---|---|
| `TGD-BL-01` | open | Name availability check for `tenantguard` | Repo creation |
...
| `TGD-BL-04` | open | Verify `casdoor` test suite runs PostgreSQL, not MySQL | Target 2 |
| `TGD-BL-05` | open | Re-baseline Tier 1 time-to-first-finding — now gated on `TGD-BL-10`, not `TGD-BL-03` | — |
...
| `TGD-BL-07` | open | Policy inference against `zitadel`'s versioned/multi-schema layout | Target 3 |
